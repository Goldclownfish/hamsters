...
def main():
    ...
    parser.add_argument(
        '-dir',
        '--directory',
        help='Directory and filename of csv. ex: accounts/hive1.csv',
        required=True)
    parser.add_argument(
        '-w',
        '--workers',
        help='amount of lines per csv',
        required=True)
    parser.add_argument(
        '-t',
        '--table',
        help='db table to get hamsters from.',
        required=True)
    args = parser.parse_args()

    QUERY = "SELECT auth,username,password FROM {} order by last_timestamp limit {};".format(
        args.table, args.workers)
    UPDATEQUERY = "UPDATE {} set last_timestamp=now() order by last_timestamp limit {}".format(
        args.table, args.workers)
